...
char* CLASSES[2] = { "__background__",
"capacitor"};
#else
char* CLASSES[4] = { "background", "Ready", "Ing",
"Touch" };
#endif
#ifdef USE_OPENCV
...
DEFINE_double(confidence_threshold, 0.00,
    "Only store detections with score higher than the threshold.");
DEFINE_double(normalize_value, 1.0,
	"Normalize image to 0~1");
DEFINE_int32(wait_time, 100,
...
int main(int argc, char** argv) {
  ...
  const string& model_file = argv[1];// "D:/SEM_SSD_5classes/project/deploy.prototxt";
  ...
  const string& indir = "D:/SEM_SSD_5classes/project/test_0.txt";//dummy
  std::string file;
  out << file_type <<"demo";
  if (file_type == "image")
  {
	  ...
	  cv::String path(argv[3]); //select only jpg
	  cv::String path_xml(argv[4]);

	  vector<cv::String> fn;
	  vector<cv::String> fn_xml;
	  vector<cv::Mat> data;

	  cv::glob(path, fn, true); // recurse
	  cv::glob(path_xml, fn_xml, true);
	  ...
	  //while (1)
	  {
		 // srand(time(NULL));
		  //int min = 0;
		  //int max = fn.size()-1;
		  //size_t k = rand()%(max-min+1)+min;
		  //printf("max=%d,%d\n", max, (int)k);
		  cv::Mat img = cv::imread(fn[k]);

		  //out
		  char fnameout[260] = "";
		  char fileOutPath[200] = "OutImage/";
		  strcpy(fnameout, fileOutPath);//path

		  ////xml reader
		  //TiXmlDocument xmlDoc(fn_xml[k].c_str());

		  //printf("%s,%s\n", fn[k].c_str(), fn_xml[k].c_str());
		  //xmlDoc.LoadFile();
		  ...
		  //TiXmlElement* pNode_2 = NULL;

		  //pNode = pRootElement->FirstChildElement("object");
		  //pNode_2 = pNode->FirstChildElement("name");
		  //if (pNode_2)
		  //{
			 // string m_sName = pNode_2->GetText();
		  //}
		  ...
		  printf("detected blobs =%d\n", detections.size());
		  /* Print the detection results. */
		
		  vector<float>& d = detections[0];
		  float tmpScore = confidence_threshold;
		  ...
		  for (int i = 0; i < detections.size(); ++i) {
			  ...
			  CHECK_EQ(d.size(), 7);
			  const float score = detections[i][2];//d[2];
		  ...
		  if (d[2] >= confidence_threshold)
		  {
				  ...
				  pt2.y = (img.rows*d[6]);
				  CvScalar color;
				  if (static_cast<int>(d[1]) == 1){//NG
					  color = cvScalar(0, 255, 0);
				  }
				  if (static_cast<int>(d[1]) == 2){//NG
					  color = cvScalar(0, 255, 255);
				  }
				  if (static_cast<int>(d[1]) == 3){//NG
				 ...
				 /* else{//OK
				  ...
				  //¥~®Ø
				  cv::rectangle(img, pt1, pt2, color, 2, 8, 0);
				  ...
				  //¤å¦r®Ø
				  cv::rectangle(img, pt1, pt3, color, -1);
				  //¤å¦r
				  cv::putText(img, label, pt1, cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 0));

			  }	  
		 // }
		  std::cout << "---" << fnameout << std::endl;
		  strcat(fnameout, fn[k].c_str());//path + image name
		  std::cout << "---" << fnameout << std::endl;
		  cv::imwrite(fnameout, img);
		  //cv::imwrite("fnameout.jpg", img);
